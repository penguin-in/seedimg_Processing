...
from torchvision.models import resnet18 as Rs18
from torchvision.models import resnet34 as Rs34
from torchvision.models import resnet50 as Rs50
from torchvision.models import resnet101 as Rs101
...
def resnet18(num_classes=1000,include_top=True,pretrained=False):
    ...
    if pretrained:
        state_dict = Rs18(pretrained=True).state_dict()
...
def resnet34(num_classes=1000,include_top=True,pretrained=False):
    ...
    if pretrained:
        state_dict = Rs34(pretrained=True).state_dict()
        model.load_state_dict({k: v for k, v in state_dict.items() if k in model.state_dict()}, strict=False)
    return model
...
def resnet50(num_classes=1000,include_top=True,pretrained=False):
    ...
    if pretrained:
        state_dict = Rs50(pretrained=True).state_dict()
        model.load_state_dict({k: v for k, v in state_dict.items() if k in model.state_dict()}, strict=False)
    return model
...
def resnet101(num_classes=1000,include_top=True,pretrained=False):
    ...
    if pretrained:
        state_dict = Rs101(pretrained=True).state_dict()
        model.load_state_dict({k: v for k, v in state_dict.items() if k in model.state_dict()}, strict=False)
    return model
