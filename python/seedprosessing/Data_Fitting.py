#This code is used to fit the distribution of seed weight changes
...
#normalized
data0_min,data0_max = data[:,0].min(),data[:,0].max()
data0_norm = (data[:,0] - data0_min) / (data0_max - data0_min)
...
#likelihood estimation for calculation of Reyleigh distribution parameters
Reyleigh_est = np.sqrt(np.sum(data0_norm**2)/(2*len(data0_norm)))
print(f'result:Reyleigh_est={Reyleigh_est:.2f}')
Reyleigh_fit = rayleigh.fit(data0_norm,floc = 0)
print(f'result:Reyleigh_fit={Reyleigh_fit[1]:.2f}')
...
pdf0_ray = rayleigh.pdf(x,loc = 0,scale = Reyleigh_est)
plt.plot(x,pdf0_ray,'g-',lw = 2,label = f"rayleigh(sigma={Reyleigh_est:.2f})")
plt.text(0.7,0.9,u"\u25A0 "+f"reyleigh:sigma={Reyleigh_est:.2f}",transform=plt.gca().transAxes,color = 'green',fontsize=16)
...
plt.title('weight growth rate distribution(normalized)',fontsize=16)
plt.xlabel('weight',fontsize=16)
plt.ylabel('number',fontsize=16)
plt.savefig("weight growth rate distribution.png", dpi=600, bbox_inches='tight')
plt.show()
